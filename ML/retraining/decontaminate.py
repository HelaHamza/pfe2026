...
----------------------------------------
Le systeme produit deja les etiquettes qui manquent : les episodes classes
...
def _load_reference_window() -> list[Incident]:
    """La fenetre de reference du gate doit rester HORS de l'entrainement.

    Sans cette exclusion, le candidat s'entraine sur [T-6mois, T], qui contient
    la fenetre de reference, alors que le modele courant -- entraine un mois
    plus tot -- ne l'a pas forcement vue. Le candidat la reconstruit donc mieux
    par simple memorisation, alerte moins, et le ratio du test (3) est biaise
    VERS LE BAS. Autrement dit : le test cense detecter un effondrement du taux
    d'alerte serait precisement celui qui le masquerait.

    En excluant la fenetre de reference de tous les corpus, aucun modele ne l'a
    jamais vue : la comparaison redevient equitable.

    Limite connue : la reference vieillit. Le modele s'en eloigne pour des
    raisons legitimes et la statistique KS derive lentement a la hausse. Il
    faut la renouveler deliberement (annuellement), en actant que les
    comparaisons anterieures ne sont plus valides.
    """
    path = Path(RC.GOLDEN_DIR) / "reference.json"
    if not path.exists():
        return []
    with open(path) as f:
        ref = json.load(f)
    return [Incident(id="__reference_window__",
                     start=_ts(ref["start"]), end=_ts(ref["end"]),
                     origin="reference")]


def load_incidents(include_golden: bool = True,
                   include_reference: bool = True) -> list[Incident]:
    """Fusionne les sources d'exclusion et deduplique."""
    incidents: list[Incident] = []
    if not RC.DECONTAMINATION_ENABLED:
        print("  [DECONTAM] *** DECONTAMINATION DESACTIVEE (bras temoin de "
              "l'experience d'empoisonnement). Le corpus contiendra les "
              "attaques confirmees. ***")
    else:
...
def mask_contaminated(df: pd.DataFrame, incidents: list[Incident],
    ...
    # --- Garde-fou anti-empoisonnement -----------------------------------
    # errors="coerce" transforme un horodatage illisible en NaT. Si la colonne
    # existe mais que 100 % des valeurs sont NaT (renommage de champ en amont,
    # '@timestamp' au lieu de 'timestamp', changement de format ES), le masque
    # serait entierement False -> ZERO evenement excise, SANS erreur. La
    # decontamination serait silencieusement desactivee : exactement le
    # scenario d'empoisonnement que toute cette couche cherche a eviter. Comme
    # ce masque est appele par chunk pendant l'extraction (extract._flush), on
    # echoue franc AVANT l'entrainement plutot que d'empoisonner le modele.
    # (Symetrique du choix deja fait cote Mongo : echouer plutot que renvoyer
    # zero incident en silence.)
    if bool(ts.isna().all()):
        raise ValueError(
            f"mask_contaminated : colonne '{ts_col}' presente mais 100 % des "
            f"horodatages sont NaT ({len(df)} lignes). Le masque serait vide "
            f"et la decontamination silencieusement inoperante (risque "
            f"d'empoisonnement). Verifie le format de '{ts_col}' / RAW_COLUMNS.")
