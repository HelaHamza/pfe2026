# IDS hôte — Couche CT (ré-entraînement mensuel)
...
Le systeme dispose déjà de la solution dans son architecture : la couche 3
...
`CNN_ARTIFACT_DIR=$(pwd)/artifacts/current` dans son unité de service,
ce qui ne demande aucune modification de code grâce au patch.

### 3.5 Timer

```bash
sudo cp deploy/cnn-retrain.* /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable --now cnn-retrain.timer
systemctl list-timers cnn-retrain
```
...
Le modèle déployé n'est pas `model_cnn.pt`. C'est un quintuplet indissociable :
