...
--------------------------------------------------------
    env["CNN_ARTIFACT_DIR"] = artifacts/_candidate
...
# ===========================================================================
def compute_window(months: int, embargo_days: int | None = None,
                   now: datetime | None = None) -> tuple[str, str]:
    """Fenetre glissante DECALEE par l'embargo.

        [ T - embargo - months ,  T - embargo ]

    L'embargo laisse a la couche Sigma/LLM le temps de confirmer les incidents
    recents AVANT que les donnees correspondantes n'entrent dans le corpus. Sans
    lui, la decontamination ne peut exciser que ce qui est deja etiquete au
    moment precis du cycle -- ce qui exclut mecaniquement les attaques les plus
    recentes, c'est-a-dire les plus susceptibles d'etre encore en cours.
    """
    now = now or datetime.now(timezone.utc)
    embargo = RC.TRAINING_EMBARGO_DAYS if embargo_days is None else embargo_days
    end = now - timedelta(days=embargo)
...
def run_training(candidate: Path, window: tuple[str, str]) -> None:
    env = {
        **os.environ,
        "CNN_ARTIFACT_DIR": str(candidate),
        "CNN_USE_CACHE": "1",
...
def run_cycle(args) -> int:
    ...
    # --- 0. preflight -----------------------------------------------------
    log.info("=" * 66)
    log.info("  CYCLE DE REENTRAINEMENT -- branche CNN")
    log.info("=" * 66)
    ...
    # --- 1. fenetre -------------------------------------------------------
    window = compute_window(args.window_months, args.embargo_days)
    log.info("[1/7] Fenetre glissante : %s -> %s (%d mois, embargo %d jours)",
             window[0], window[1], args.window_months, args.embargo_days)
    log.info("      L'embargo laisse a la couche 3 le temps de confirmer les "
             "incidents recents avant qu'ils n'entrent dans le corpus.")
    ...
    # Metriques cles recopiees DANS le manifest : c'est ce qui rend
    # `artifact_store --history` possible sans relire tous les rapports, et
    # donc ce qui permet de voir le cliquet (decroissance lente du taux
    # d'alerte sur plusieurs cycles) plutot que de le subir.
    by_test = {t["nom"]: t.get("metriques", {}) for t in report["tests"]}
    key_metrics = {
        "episodes_reference": by_test.get("taux_alerte", {})
                                     .get("episodes_candidat"),
        "ratio_vs_courant": by_test.get("taux_alerte", {}).get("ratio"),
        "golden_recall": by_test.get("golden_set", {}).get("recall"),
        "ks_D_max": by_test.get("distribution_scores", {}).get("D_max"),
        "seuils": by_test.get("seuils_pot", {}).get("seuils"),
    }
    dest = AS.promote(candidate, version, {
        "metriques": key_metrics,
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(
        description="Cycle mensuel de reentrainement du CNN")
    ap.add_argument("--window-months", type=int, default=RC.RETRAIN_WINDOW_MONTHS)
    ap.add_argument("--embargo-days", type=int, default=RC.TRAINING_EMBARGO_DAYS,
                    help="jours les plus recents exclus du corpus "
                         "(delai de decouverte des incidents)")
