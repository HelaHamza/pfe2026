...
# ===========================================================================
def _history() -> int:
    """Historique des metriques cycle apres cycle -- detecteur de CLIQUET.

    Le risque le plus insidieux d'une boucle de reentrainement fermee n'est pas
    une panne franche : c'est une DECROISSANCE LENTE du taux d'alerte. Chaque
    cycle pris isolement passe le gate (le ratio reste dans [0.5x, 2x]), mais
    0.8x repete six fois de suite donne 0.26x. Le modele se rend
    progressivement aveugle, et aucun test mono-cycle ne peut le voir.

    Seule la SERIE le revele. D'ou cette commande.
    """
    root = artifacts_root()
    versions = list_versions()
    if not versions:
        print("Aucune version promue.")
        return 0

    print(f"{'version':<14} {'episodes':>9} {'ratio':>7} {'golden':>7} "
          f"{'KS D':>6}  config")
    print("-" * 62)
    series = []
    for v in versions:
        man = read_manifest(root / v) or {}
        m = man.get("metriques") or {}
        ep = m.get("episodes_reference")
        ratio = m.get("ratio_vs_courant")
        rec = m.get("golden_recall")
        ks = m.get("ks_D_max")
        fp = (man.get("config_fingerprint") or {}).get("hash", "?")
        if ep is not None:
            series.append(ep)
        print(f"{v:<14} {_fmt(ep, '{:>9,}'):>9} {_fmt(ratio, '{:>7.2f}'):>7} "
              f"{_fmt(rec, '{:>6.0%}'):>7} {_fmt(ks, '{:>6.3f}'):>6}  {fp}")

    if len(series) >= 3:
        drift = series[-1] / series[0] if series[0] else float("nan")
        decreasing = all(b <= a for a, b in zip(series, series[1:]))
        print("-" * 62)
        print(f"Derive cumulee sur {len(series)} cycles : x{drift:.2f}")
        if decreasing and drift < 0.6:
            print("  ! CLIQUET PROBABLE : le taux d'alerte decroit a chaque "
                  "cycle. Chaque cycle passe le gate isolement, mais la serie "
                  "montre un modele qui se rend aveugle.")
            print("  ! Verifier : les episodes false_positive sont-ils bien "
                  "CONSERVES dans le corpus ? (cf. regle des trois verdicts)")
            return 1
    return 0
...
def main(argv=None) -> int:
    import argparse
    ap = argparse.ArgumentParser(description="Store d'artefacts IDS hote - branche CNN")
