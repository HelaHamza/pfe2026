"""
evaluation.py : concerne partie test
=============
...
# ═══ Persistance Mongo (best-effort, dépendance ML → backend isolée ici) ═══
def _resolve_version(explicit, here):
    """Version = symlink artifacts/current → 2026-07-01. --version pour forcer."""
    if explicit:
        return explicit
    current = os.path.join(here, "artifacts", "current")
    try:
...
def _persist_to_mongo(M, version):
    """Passe M au MetricsRepository du backend (schéma défini là-bas, une
    seule fois). Un échec (backend absent, Atlas down) N'INTERROMPT PAS
    l'éval : le JSON reste écrit, ré-ingérable via scripts.ingest_metrics."""
    backend = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "backend"))
    if backend not in sys.path:
        sys.path.insert(0, backend)
    try:
        from core.timeutils import now_utc
        from models.metrics_model import EvalMetrics, MetricsDocument
        from repositories.metrics_repository import MetricsRepository
    except Exception as e:
        print(f"  /!\\ backend non importable ({e}) → Mongo ignoré, JSON seul.")
        return
    try:
        metrics = EvalMetrics.model_validate(M)
        doc = MetricsDocument(version=version, ingested_at=now_utc(),
                              source_csv=metrics.config.from_csv, metrics=metrics)
        MetricsRepository.insert_version(doc)
        print(f"  ✓ Métriques v{version} → Mongo (collection 'metrics').")
    except Exception as e:
        print(f"  /!\\ écriture Mongo échouée ({e}) → JSON conservé.")
...
def main():
    ...
    ap.add_argument("--version", default=None,
                    help="version modèle ; défaut = symlink artifacts/current")
    ap.add_argument("--no-mongo", action="store_true",
                    help="désactive l'écriture Mongo (JSON seul)")
    args = ap.parse_args()
    ...
    with open(out, "w") as f:
        ...
        if not args.no_mongo:
            version = _resolve_version(
                args.version, os.path.dirname(os.path.abspath(__file__)))
            _persist_to_mongo(M, version) # stockage dans Mongo (collection 'metrics') pour dashboard backend
    print(f"\n  -> {out}")
