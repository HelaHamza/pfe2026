...
CONSOLIDATION (cette version) :
  1. SOURCE UNIQUE DE PROMPT -- prompts_cnn (SYSTEM_PROMPT / FEWSHOT /
     OUTPUT_SCHEMA / build_user_prompt). Il n'existe qu'un seul prompt, celui
     que le LLM recoit reellement.
  2. COUCHE LLM PURE -- aucun garde-fou deterministe de severite a cet etage.
     La severite d'un episode est celle que le LLM decide ; _clamp_severity
     garantit seulement qu'elle reste dans l'echelle CL.SEVERITIES. La detection
     des primitives sensibles (creation de compte, cron...) releve de Sigma,
     pas de la couche d'explication.
  3. GROUNDING APPLIQUE -- grounding_cnn nettoie la SORTIE sans jamais fermer
     ...
     kb_refs non montres ecartes, evidence non verifiable signalee.
  4. SEVERITE NORMALISEE -- seul signal de tri restant en mode explication :
     garantie dans l'echelle CL.SEVERITIES. C'est la severite du LLM, non
     contrainte par un plancher.
...
evidence_suspects) sont PUREMENT observationnelles : l'aval (adaptateur, Mongo,
API, dashboards) peut les ignorer sans rien casser.
...
def _clamp_severity(sev) -> str:
    """Severite = SEUL signal de tri en mode explication. On garantit UNE seule
    chose : la valeur reste DANS l'echelle CL.SEVERITIES (sinon le tri du
    dashboard casse). Une valeur inconnue est ramenee a 'medium' -- sur un outil
    de securite, on ne sous-priorise pas par defaut.

    Aucun plancher n'est impose : la severite est celle que le LLM a decidee
    (couche d'explication pure). La detection des primitives sensibles releve
    de Sigma, pas de cet etage.
    """
...
def triage_rag(ep: EC.Episode, index: rag_cnn.KBIndex,
               dossier: str) -> tuple[dict, list[str]]:
    """Retourne (explication_brute, ids_kb_montres).

    Aucun champ 'verdict' n'est lu du LLM : _build_record le fixe a
    CL.ALERT_VERDICT. `dossier` est calcule EN AMONT (il ne fait aucun appel
    reseau) pour rester disponible meme sur le chemin fail-open.
    """
    hits = index.retrieve(ep.rag_query(), ep.keys, ep.log_source)
    shown_ids = [c.id for c, _ in hits]
    kb_block = index.render(hits)
    user = P.build_user_prompt(dossier, kb_block, index.allowed_mitre)
...
def _build_record(ep: EC.Episode, raw: dict, shown: list[str],
                  dossier: str, index: rag_cnn.KBIndex) -> dict:
...
def main() -> None:
    ...
    for i, ep in enumerate(eps, 1):
        # dossier : calcule AVANT l'appel LLM (aucun reseau) -> reste disponible
        # pour _build_record meme si le LLM echoue (fail-open).
        dossier = ep.render()
        try:
            raw, shown = triage_rag(ep, index, dossier)
        ...
        rec = _build_record(ep, raw, shown, dossier, index)
        results.append(rec)
