...
def _cnn_branch(run_id: str, until: str) -> tuple[list[dict], str]:
    ...
    _log("Inférence CNN + explication LLM en cours… (plusieurs minutes)")
...
# ══════════════════════════════════════════════════════════════════════
#  Auto-ingest hors-bande : domaine C (ré-entraînement).
#  Non bloquant : un échec ici n'invalide JAMAIS le run de détection.
# ══════════════════════════════════════════════════════════════════════
...
def _save_snapshot(run_id, started, status, errors,
                   cnn_eps, sigma_alerts, logs_by_source):
    # Mode explication seule : le LLM n'écarte plus rien. Chaque épisode levé
    # par le CNN est conservé et expliqué (verdict constant true_positive).
    # Plus de bruit filtré, plus de file de revue → ces compteurs valent 0.
    cnn_sev = Counter(str(e.get("severity", "low")).lower() for e in cnn_eps)
    cnn_verdict = Counter(e.get("verdict", "true_positive") for e in cnn_eps)
    sig_lvl = Counter(str(a.get("level", "LOW")).lower() for a in sigma_alerts)

    # Anomalies AE ventilées par source. INITIALISÉ à 0 pour CHAQUE source
    # présente dans les logs : un run à 0 anomalie renvoie {auditd:0, auth:0,
    # syslog:0} et non {} → le front distingue « 0 » de « champ absent ».
    ...
    report = Report(
        ...
        stats=ReportStats(
            ...
            cnn_kept=len(cnn_eps),      # tous conservés (aucun filtrage LLM)
            cnn_to_review=0,            # plus de file de revue
            ...
            noise_reduction_pct=0.0,    # le LLM ne filtre plus rien
        ),
        cnn_by_severity=dict(cnn_sev),
        cnn_by_verdict=dict(cnn_verdict),   # = {"true_positive": N}
